...
//                    TRABALHO FINAL
//
// [00278880] Matheus Saueressig e [00302072] Victória Duarte
...
// Booleanos que definem se as teclas de movimentação da câmera estão pressionadas
// "g_PressingKey[tecla] = true" se o usuário pressiona a tecla [A, W, S ou D].
bool g_PressingKeyD = false;
bool g_PressingKeyA = false;
bool g_PressingKeyW = false;
bool g_PressingKeyS = false;


// Booleano que define se a câmera será livre ou de look-at
bool g_Key0Pressed = false;
...
{   
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - Trabalho Final - Matheus e Victória", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    ObjModel cutiflymodel("../../data/cutifly.obj");
    ComputeNormals(&cutiflymodel);
    BuildTrianglesAndAddToVirtualScene(&cutiflymodel);
    ...
    // Variáveis que definem a câmera virtual
    glm::vec4 camera_position_c  = initial_c_position; // Ponto "c", centro da câmera
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if(g_Key0Pressed) {
            camera_view_vector = camera_look_at_point - camera_position_c;
        } else {
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mod)
{
    ...
    // Se o usuário apertar a tecla 0, fazemos um "toggle" com a câmera Look At
    if (key == GLFW_KEY_0 && action == GLFW_PRESS) {
        g_Key0Pressed = !g_Key0Pressed;
    }

    // Se o usuário apertar a tecla D, ativamos a movimentação para FRENTE
    if (key == GLFW_KEY_W) {
        if(action == GLFW_PRESS) {
            g_PressingKeyW = true;
        } else if(action == GLFW_RELEASE) {
            g_PressingKeyW = false;
        }
    }

    // Se o usuário apertar a tecla S, ativamos a movimentação para TRÁS
    if (key == GLFW_KEY_S) {
        if(action == GLFW_PRESS) {
            g_PressingKeyS = true;
        } else if(action == GLFW_RELEASE) {
            g_PressingKeyS = false;
        }
    }

    // Se o usuário apertar a tecla A, ativamos a movimentação para ESQUERDA
    if (key == GLFW_KEY_A){
        if(action == GLFW_PRESS) {
            g_PressingKeyA = true;
        } else if(action == GLFW_RELEASE) {
            g_PressingKeyA = false;
        }
    }

    // Se o usuário apertar a tecla D, ativamos a movimentação para DIREITA
    if (key == GLFW_KEY_D) {
        if(action == GLFW_PRESS) {
            g_PressingKeyD = true;
        } else if(action == GLFW_RELEASE) {
            g_PressingKeyD = false;
        }
    }
